...
export function ElementBody(elementId) {
  ...
  let choices = getElementMetadata(elementId).choices ?? [];
  choices = choices.map((i) => ({ ...i, text: i.text }));
  ...
  const handleUpdateMeta = (index: number) => (value: ChoiceField) => {
    if (mode === WorkbenchMode.FILLOUT) {
      value.choices.forEach((_i, idx) => {
        setValue(
          `choices.${idx}.checked`,
          idx !== index ? false : !!!value.choices[index]?.checked,
        );
      });
