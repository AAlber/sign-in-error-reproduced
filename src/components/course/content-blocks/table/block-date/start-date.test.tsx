...
describe("BlockDate Component", () => {
  ...
  it("adds start date", async () => {
    ...
    render(<StartDate block={dummyBlock} />);
    const addButton = await screen.findByRole("button", { name: /add start/i });
    ...
    expect(state?.startDate?.getDate()).toBe(date);
  });
});
